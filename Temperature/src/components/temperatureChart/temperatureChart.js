...
export class TemperatureChart extends Component {
     ...
     */
    changeBeginDay = (PageIndex, IsLast7Days,flag) => {
        if(flag == 'last' || flag == 'forward'){
            if((this.state.PageIndex  == this.state.PageCount)){
                message.info("已经是最后一周");
                return;
            }
        }
        if(flag == 'first' || flag == 'back'){
        ...
        this.props.changeBeginDay(PageIndex,IsLast7Days);
    }
    /**
     * 时间控件切换开始开始
     * @param date
    ...
    render() {
        let {beginDate, patientList, hospital, dayOps, dayList, pointTime, breathingList, dayMap, loading,PageCount,
...
onClick={this.changeBeginDay.bind(this, 1, 0,'first')}>第一周</Button>
<Button type="primary" size="small" onClick={this.changeBeginDay.bind(this, PageIndex, 0,'back')}>上一周</Button>
<Button type="primary" size="small" onClick={this.changeBeginDay.bind(this, PageIndex, 0,'forward')}>下一周</Button>
<Button type="primary" size="small"
onClick={this.changeBeginDay.bind(this, PageCount, 0,'last')}>最后一周</Button>
</ButtonGroup>
<span style={pageNum}>第{PageIndex}页</span>
</div>
