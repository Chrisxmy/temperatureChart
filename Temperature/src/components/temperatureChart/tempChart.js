    ...
    for (let i = 0; i < oArray.length; i++) {
        ...
        switch (oArray[i]) {
            ...
            case "mb" :
                ...
                break;//脉搏
            case "xl" :
                pTuglie.push(drawCircleDot("xl", {"x": initX, "y": (initY + i * 16)}, 4, "red", "心率", false));
...
export function drawEvent(evenArray, curDate) {
    ...
    y={12 + i * height} fill="blue">{v}</text>
    })}
</svg>
)
    }
...
for (let i = 0; i < coor.length; i++) {
    ...
    for (let j in json) {
        ...
        let yw = json[j].wd.type === 'TPR001T12'; //腋温
        let kw = json[j].wd.type === 'TPR001T11'; //口温
        let gw = json[j].wd.type === 'TPR001T13'; //肛温
        //判断是否重合
...
function drawCircleDot(type, xy, r, color, title, fill) {
    if (title === null || title === undefined) {
        return "";
    }
    let width = 3;	//定义圆环的宽度
