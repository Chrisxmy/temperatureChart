...
export class GridTable extends Component {
    ...
    createTable = (breathingList, dayMap) => {
        ...
        for (let i = 0; i < 42; i++) {
            ...
            if ((i + 1) % 6 === 0) {
                gridArray[len].push(<td style={{verticalAlign: align}} key={i + 1}
                                        className="borderR hx">{hxData[i]}</td>);
            } else {
        ...
        for (let i = 0; i < dayMap.length; i++) {
            ...
            gridArray[length] = [];
            gridArray[length].push(<td key={0} className="every">{dayMap[i].name + "(" + dayMap[i].units + ")"}</td>);
            for (let j = 0; j < dayMap[i].items.length; j++) {
                gridArray[length].push(<td key={1 + j} colSpan={6} className="every borderR">{dayMap[i].items[j]}</td>);
            }
            gridArray[length].push(<td key={dayMap[i].items.length + 1} className="every"/>)
        }
        return gridArray;
    }
